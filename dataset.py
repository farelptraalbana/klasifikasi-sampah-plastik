...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.all_annot_paths = glob.glob(
            os.path.join(self.labels_path, '*.xml'))
        self.all_images = [image_path.split(
            os.path.sep)[-1] for image_path in self.all_image_paths]
    ...
    def read_and_clean(self):
        ...
        for annot_path in self.all_annot_paths:
            ...
            if object_present == False:
                ...
                print(
                    f"Removing {annot_path} and corresponding {remove_image}")
        ...
        for image_name in self.all_images:
            possible_xml_name = os.path.join(
                self.labels_path, image_name.split('.jpg')[0]+'.xml')
    ...
    def load_cutmix_image_and_boxes(self, index, resize_factor=512):
        ...
        xc, yc = [int(random.uniform(h * 0.25, w * 0.75))
                  for _ in range(2)]  # center x, y
        indexes = [index] + \
            [random.randint(0, len(self.all_images) - 1) for _ in range(3)]
        ...
        result_boxes = result_boxes[
            np.where((result_boxes[:, 2]-result_boxes[:, 0])
